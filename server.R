...
# Polling function to check the terminal output and return it
    ...
    if (Sys.getenv("RSTUDIO") == "1") {
      ...
      if (length(start_idx) > 0) {
        if (length(end_idx) > 0) {
          ...
          print(output)  # Update the reactive value when the task is completed
        } else {
...
# Start the DeepTMHMM code when the action button is clicked
observeEvent(input$start_deeptmhmm, {
  ...
  if (Sys.getenv("RSTUDIO") == "1") {
    ...
    setwd(temp_dir)
    system(paste("cd", temp_dir, "&& biolib run DTU/DeepTMHMM --fasta", temp_fasta, ">", file.path(temp_dir, "output_log.txt"), "2>&1 &"))
  }
  
  # Reactive function that updates the terminal output in real-time
...
# Render the result image after the output is completed
observeEvent(output_completed(), {
  setwd(temp_dir)
  
  output$tmhmm_plot <- renderImage({
    filename <- file.path(temp_dir, 'biolib_results', 'plot.png')
