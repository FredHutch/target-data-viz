...
circosPlotUI <- function(id, label = "Circos plot parameters"){
  ...
  all_fusions <- list("RUNX1-RUNX1T1", "MLLT10-X", "NUP98-X", "DEUP1-EYA1", "CADM2-MGAM", "CSNK1G1-OR6J1", "DEUP1-RALYL", "GNPDA1-ABCC2", "PTPRR-COL6A6", "RUNX1-KCNB2", "RUNX1-LRRC69")
  names(all_fusions) <- list("RUNX1-RUNX1T1", "MLLT10-X", "NUP98-X", "DEUP1-EYA1", "CADM2-MGAM", "CSNK1G1-OR6J1", "DEUP1-RALYL", "GNPDA1-ABCC2", "PTPRR-COL6A6", "RUNX1-KCNB2", "RUNX1-LRRC69")
  ...
  fluidPage(
    ...
    sidebarPanel(
      ...
      helpText(" "),
      helpText(" "),
      helpText("Translocations indicative of canonical pediatric AML fusions (NUP98-X, RUNX1-RUNX1T1, and MLLT10-X) are depicted in blue."),
      helpText(" "),
      helpText(" "),
      helpText(" "),
      helpText(" "),
...
circosPlot <- function(input, output, session){
  ...
  patient_fusion_dt <- readRDS("./data/ptdf.rds")
  ...
  cp_objs <- eventReactive(input$updateplot, {
    ...
    # user chooses to view TWO chromosomes ---------------------------------------------------------------------------------
      ...
      for(i in 1:length(pts_with_fusion)) {
        ...
        #TARGET-20-PAUZRY-09A-02D_consensus -> RUNX1 gene on chromosome 21 with RUNX1T1 gene on chromosome 8
        if (pat == "TARGET-20-PAUZRY-09A-02D"){
          bed_BND_CANON <- data.frame(chr= c("chr21"), chr_mate = c("chr8"), pos = c(34838337), pos =c(34838337), gene= c("RUNX1"), value=c(1))
          bed_BND_MATES_CANON <- data.frame(chr_mate = c("chr8"), chr= c("chr21"), pos = c(92058671), pos = c(92058671), gene= c("RUNX1T1"), value=c(1))
        }
        ...
        if (pat == "TARGET-20-PAURDN-03A-01D"){
          ...
          bed_BND_CANON <- data.frame(chr= c("chr11"), chr_mate = c("chr5"), pos = c(3743985), pos =c(3743985), gene= c("NUP98"), value=c(1))
          bed_BND_MATES_CANON <- data.frame(chr_mate = c("chr5"), chr= c("chr11"), pos = c(177233312), pos = c(177233312), gene= c(" "), value=c(1))
        ...
        if (pat == "PAVESI-03A-01D"){
          ...
          gene <- c("NUP98", "NUP98")
          gene_mate <- c("", "")
          bed_BND_CANON <- data.frame(chr, chr_mate,  pos, pos, gene, value)
          bed_BND_MATES_CANON <- data.frame(chr_mate, chr,  pos_mate, pos_mate, gene_mate, value)
        }
